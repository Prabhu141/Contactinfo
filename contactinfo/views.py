...
def extract_contact_info(url):
    ...
    # Extract company name using regex pattern from the text content
    company_name_pattern = r'https://www\.(.*?)\.com/'
    company_names = re.findall(company_name_pattern, url , re.IGNORECASE)

    # Extract address using regex pattern
    address_pattern = r'\b(\d+\s[\w\s\#]+,\s[\w\s]+,\s[\w\s]+)\b'
    details = re.findall(address_pattern,soup.get_text())
    print("Emails:", emails)
    print("Phones:", phones)
    print("Company Names:", company_names)
    print("Addresses:", details)
    return emails, phones, company_names, details

# Function to process the sitemap XML file and extract contact information
def process_sitemap(sitemap_path):
    ...
    for url_element in root.findall('.//{http://www.sitemaps.org/schemas/sitemap/0.9}url'):
        ...
        if 'contact' in url.lower():
            emails, phones, company_names, details = extract_contact_info(url)

            if emails or phones or company_names or details:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            contact_info = ContactInfo.objects.create(
                ...
                emails=', '.join(emails),
                phones=', '.join(phones),
                companyname=', '.join(company_names),
